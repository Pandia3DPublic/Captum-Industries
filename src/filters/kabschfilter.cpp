...
//void dualKabschFilter(std::vector<match>& matches, Eigen::Matrix4d& trans) {
//	std::vector<match> matches_bu = matches;
//	std::vector<match> matches_td = matches;
//	Eigen::Matrix4d trans_bu = Eigen::Matrix4d::Zero();
//	Eigen::Matrix4d trans_td = Eigen::Matrix4d::Zero();
//	buKabschFilter(matches_bu, trans_bu);
//	tdKabschFilter(matches_td, trans_td);
//
//	//defaults to top down
//	if (matches_bu.size() > matches_td.size()) { //bottom up kabsch has more matches and is stable
//		trans = trans_bu;
//		matches = matches_bu;
//	} else {//top down kabsch has more matches and is stable
//		trans = trans_td; 
//		matches = matches_td;
//	}
//
...
//		matches = empty;
//		trans = Eigen::Matrix4d::Zero();
//		utility::LogDebug("All matches removed by kabsch filter! \n");
//	}
//
//
//}
...
void kabschfilter(std::vector<match>& matches, pairTransform& trans) {
	...
	if (fm.size() >= mincors) {
		...
		trans.kabschtrans = gettrans(b_ctr)*Rh*gettrans(a_ctr).inverse();
		trans.invkabschtrans = trans.kabschtrans.inverse();
		//Eigen::Matrix3d eye = Eigen::Matrix3d::Identity();
		//Eigen::Quaterniond eyeq(eye);
		//Eigen::Quaterniond kabschq(Rh.block<3,3>(0,0));
		//Eigen::Quaterniond midq = eyeq.slerp(0.5,kabschq);


		//Eigen::Matrix3d midsmall(midq);
		//Eigen::Matrix4d Rmid =  Eigen::Matrix4d::Identity(); //rotation only
		//Rmid.block<3,3>(0,0) = midsmall;
		//Rmid(3,3) =1;
		//Eigen::Vector3d dthalf = trans.kabschtrans.block<3,1>(0,3)/2;

		//trans.Tk2A = Rmid;
		//trans.Tk2A.block<3,1>(0,3) = dthalf;
		////need to do this since rotation is only valid for reference system of pcd a.
		//trans.Tk2B =gettrans(dthalf)* Rmid.inverse() * gettrans( -trans.kabschtrans.block<3,1>(0,3)) ;
		trans.set = true;
