...
#include "ICPUtil.h"
#include "SolverEquationsLie.h" //for residual stuff
...
__inline__ __device__ float evalFDevice(unsigned int corrIdx, SolverInput& input, SolverState& state, SolverParameters& parameters, bool debug)
{
	...
	if (corr.isValid()) {
		float3x3 TI = evalRMat(state.d_xRot[corr.imgIdx_i]); //correct vars?
		float3x3 TJ = evalRMat(state.d_xRot[corr.imgIdx_j]);
		//printf("%f %f %f \n", state.d_xRot[corr.imgIdx_i].x,state.d_xRot[corr.imgIdx_i].y,state.d_xRot[corr.imgIdx_i].z);
		//TI.print();
		//TJ.print();
		//printf("\n");
		//test result: For zero dofs matrices are good, yet residual is still wrong
		//printf("\n");
		//points look okay, no nan, etc.

		r = (TI*corr.pos_i + state.d_xTrans[corr.imgIdx_i]) - (TJ*corr.pos_j + state.d_xTrans[corr.imgIdx_j]);
		//printf("%f \n", parameters.weightSparse); always 1.0
		float res = parameters.weightSparse * dot(r, r);
		//printf("single residual between %u and %u: %f. Points are %f %f %f and %f %f %f \n", corr.imgIdx_i,corr.imgIdx_j, res,corr.pos_i.x,corr.pos_i.y,corr.pos_i.z,corr.pos_j.x,corr.pos_j.y,corr.pos_j.z);
		//printf("%f %f %f \n", corr.pos_i.x,corr.pos_i.y,corr.pos_i.z);
		//printf("%f %f %f \n", corr.pos_j.x,corr.pos_j.y,corr.pos_j.z);
		return res;
	}
	return 0.0f;
}
...
__inline__ __device__ float computeColorDProjLookup(const float4& dx, const float4& camPosSrcToTgt, const float2& intensityDerivTgt, const float2& colorFocalLength)
{
	...
	float3 tmp = make_float3(camPosSrcToTgt.x,camPosSrcToTgt.y,camPosSrcToTgt.z);
	//todo why was tmp not here all along (script kidi protection)
	mat2x3 dProjectionC = dCameraToScreen(tmp, colorFocalLength.x, colorFocalLength.y);
