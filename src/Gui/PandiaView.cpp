...
void PandiaView::SetViewMatrices( const Eigen::Matrix4d  &model_matrix /* = Eigen::Matrix4d::Identity()*/) {
	...
	if (GetProjectionType() == ProjectionType::Perspective) {
		...
		z_near_ = constant_z_near_ > 0? constant_z_near_: std::max(0.01 * bounding_box_.GetMaxExtend(),	distance_ -	3.0 * bounding_box_.GetMaxExtend());
		z_far_ = constant_z_far_ > 0? constant_z_far_: distance_ + 3.0 * bounding_box_.GetMaxExtend();	
		projection_matrix_ =GLHelper::Perspective(field_of_view_, aspect_, z_near_, z_far_);
	} else {
		// Orthogonal projection
		// We use some black magic to support distance_ in orthogonal view
		z_near_ = constant_z_near_ > 0	? constant_z_near_	: distance_ - 3.0 * bounding_box_.GetMaxExtend();
		z_far_ = constant_z_far_ > 0? constant_z_far_	: distance_ + 3.0 * bounding_box_.GetMaxExtend();
		projection_matrix_ =GLHelper::Ortho(-aspect_ * view_ratio_, aspect_ * view_ratio_,
...
void PandiaView::setBoundingBox(std::vector<Eigen::Vector3f>& points) {
...
void PandiaView::setVirtualIntrinsic() {
	...
	auto& v_intrinsic = virtualIntrinsic.intrinsic_matrix_;
	...
	virtualIntrinsic_cuda = open3d::cuda::PinholeCameraIntrinsicCuda(virtualIntrinsic);
}
