...
namespace PandiaGui {
	...
	void window_Menubar(PandiaView& v) {
		...
		if (ImGui::BeginMainMenuBar()) {
			...
			if (ImGui::BeginMenu("Shader")) {
				...
				if (loadOptionSwitch != 0)
					ImGui::RadioButton("RayCast", &shadertyp, 3);
				ImGui::Separator();
	...
	void window_WarmUp() {
		if (g_warmupCamera) {
			...
			ImGui::TextColored(PandiaGui::col_yellow, PandiaGui::lan_warmUp.c_str());
	...
	void window_CameraConnection() {
		if (!g_cameraConnected && !g_cameraParameterSet) {
			...
			ImGui::TextColored(PandiaGui::col_red_3, PandiaGui::lan_noConnection.c_str());
			ImGui::End();
		}
	}
	...
	void window_PostProcessing() {
		if (PandiaGui::g_postProcessing) {
			...
			if (PandiaGui::pp_denseAlign) {
				...
				ImGui::TextColored(PandiaGui::col_yellow, PandiaGui::lan_voxelLength.c_str());
				ImGui::ProgressBar(PandiaGui::currentProgress / PandiaGui::overallPostProgress, ImVec2(-1.f, 0.f)); //shows user progressbar in Postprocessing
				...
				ImGui::TextColored(PandiaGui::col_yellow, PandiaGui::lan_meshReduction.c_str());
				ImGui::ProgressBar(PandiaGui::openCurrentProgress / PandiaGui::openOverallProgress, ImVec2(-1.f, 0.f)); //shows progressbar in Postprocessing
			}
			ImGui::SetWindowSize(miniWindow);
			ImGui::SetWindowPos(ImVec2(PandiaGui::topleft.x + PandiaGui::renderWindowSize.x / 2 - miniWindow.x / 2, PandiaGui::topleft.y + PandiaGui::renderWindowSize.y / 2 - miniWindow.y));
			ImGui::End();
	...
	void window_CameraLegend(Model& m) {
		if (PandiaGui::menu_showCameraPath && !m.chunks.empty() && g_programState == gui_PAUSE) {
			...
			ImGui::Text(PandiaGui::lan_cameraLegend.c_str());
			ImGui::End();
			style = style_old;
		}
	}
	...
	void window_OpenGL(PandiaView& v) {
		...
		if (!g_current_slam_finished && g_programState == gui_RUNNING) { //only show indicator if reconstruction is in progress 
			ImDrawList* drawlist = ImGui::GetWindowDrawList();
			if (!g_trackingLost) { //tracking ok
	...
	void window_SaveImageTrajectory(Model& m) {
		...
		if (PandiaGui::saveImageFlag) {
			ImGui::Spacing();
			ImGui::Text(PandiaGui::lan_InfoImageSave.c_str());
			ImGui::PushStyleColor(ImGuiCol_Text, PandiaGui::col_yellow);
			ImGui::TextWrapped(PandiaGui::saveImagePath.c_str());
			ImGui::PopStyleColor();
		...
		if (PandiaGui::saveTrajectoryFlag) {
			...
			ImGui::PushStyleColor(ImGuiCol_Text, PandiaGui::col_yellow);
			ImGui::TextWrapped(PandiaGui::saveTrajectoriesPath.c_str());
			ImGui::PopStyleColor();
