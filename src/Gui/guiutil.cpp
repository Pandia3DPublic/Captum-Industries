...
namespace PandiaGui {
	...
	shared_ptr<geometry::TriangleMesh> cpumesh = make_shared<geometry::TriangleMesh>();
	...
	void initPandiaGui() {
		...
		setVoxelSliderValue(); //sets voxelLength on runtime, else initial voxel-length in postprocessing-menu is not valid
	...
	void GuiBlockItem(var& disable) {
		...
		if (tmp) {
			...
			auto tmpHeight = absolutWindowPos.y - pos.y;
			...
			ImGui::Begin(s.c_str(), &tmp, disableInput_flags);
	...
	void internal_GuiBlockItem_TemplateFunction() { // needs to exist to generate all possible template functions
		bool a;
		std::atomic<bool> b;
		ImVec2 size;

		GuiBlockItem<bool>(a);
		GuiBlockItem<std::atomic<bool>>(b);
		GuiBlockItem<bool>(a, size);
		GuiBlockItem<std::atomic<bool>>(b, size);
	}
	...
	//############################################### PandiaClock Class begin #############################################
	PandiaClock::PandiaClock(bool start) {
		if(start)
	...
	double PandiaClock::getDuration(PandiaTimeUnit unit) {

		if (isRunning) {

			switch (unit)
			{
			case PandiaGui::PandiaTimeUnit::hours:
				current = std::chrono::steady_clock::now();
				auto hours = std::chrono::duration_cast<std::chrono::hours>(current - begin);
				return hours.count();
				break;

			case PandiaGui::PandiaTimeUnit::minutes:
				current = std::chrono::steady_clock::now();
				auto tmp = std::chrono::duration_cast<std::chrono::minutes>(current - begin);
				return tmp.count();
				break;

			case PandiaGui::PandiaTimeUnit::seconds:
				current = std::chrono::steady_clock::now();
				duration = std::chrono::duration_cast<std::chrono::seconds>(current - begin);
				break;

			case PandiaGui::PandiaTimeUnit::milliseconds:
				current = std::chrono::steady_clock::now();
				duration = std::chrono::duration_cast<std::chrono::milliseconds>(current - begin);
				break;
	...
	//returns time passed as string
	string PandiaClock::getClock() {
		
		if (isRunning) {
			clock = "";
			timePassed = getDuration(PandiaTimeUnit::milliseconds);
			timePassed += offset; //to pick up time where left off
			clock = to_string(((int)timePassed / 3600));
			if (clock.size() == 1)
				clock = "0" + clock;
			clock += " : ";

			string minutes = to_string(((int)timePassed / 60));
			if (minutes.size() == 1)
				clock += "0" + minutes;
			else 
				clock += minutes;
			...
			string seconds = to_string(((int)timePassed % 60));
			if (seconds.size() == 1)
				clock += "0" + seconds;
			else 
				clock += seconds;
	...
	void PandiaClock::saveTimestamp() {
		timestamp.push_back(std::chrono::high_resolution_clock::now());
	}
	//############################################### PandiaClock Class end #############################################
	...
	void showDebugWindow(Model& m, PandiaView& v){
		...
		if (ImGui::CollapsingHeader("MODEL")) {
			int no_chunks = m.chunks.size();			
			ImGui::TextColored(PandiaGui::col_orange, "MODEL");
			ImGui::Text("Chunks: %d", no_chunks);
			ImGui::Text("invalid Chunks: %d", m.invalidChunks.size());
			if (no_chunks != 0) {
				ImGui::Text("efficient Matches: %d", m.chunks[no_chunks - 1]->efficientMatches.size());
			}
			else {
				ImGui::Text("efficient Matches: NAN");
			}
			ImGui::Text("filtered Matches: %d", m.filteredmatches.x.size());
			ImGui::Text("raw Matches: %d", m.rawmatches.x.size());
			ImGui::Text("frame-ID: %d", frame_id_counter.id);
			ImGui::Text("chunk-ID: %d", chunk_id_counter.id);
			ImGui::Text("Current mesh size (vertices):  %d", m.mesher.mesh_.vertices_.size());
			ImGui::Text("Current mesh size (color):     %d", m.mesher.mesh_.vertex_colors_.size());
			ImGui::Text("Current mesh size (triangles): %d", m.mesher.mesh_.triangles_.size());

				
			if (ImGui::TreeNode("TSDF volume Cuda")) {

				if (m.tsdf_cuda.device_ != nullptr) { //prevents crasing if no tsdf exists (like if postprocessing is activated)
					auto tmpSubvolumes = m.tsdf_cuda.active_subvolume_entry_array_.size();
					ImGui::Text("active subvolumes: %d", tmpSubvolumes);
				}
				ImGui::Text("value capacity: %d", m.tsdf_cuda.value_capacity_);
				ImGui::Text("bucket count: %d", m.tsdf_cuda.bucket_count_);
				ImGui::Text("voxel lenght: %f", m.tsdf_cuda.voxel_length_);
				ImGui::Text("unmeshed data: %d", (bool)m.tsdf_cuda.unmeshed_data);
				ImGui::Text("truncation: %f", m.tsdf_cuda.sdf_trunc_);
			...
			if (ImGui::TreeNode("Visualisation")) {
				...
				//saves efficient Match size of chunks for graphic
				for (int i = 0; i < no_chunks; i++) {
					vec_efficientMatchChunk.push_back(m.chunks[i]->efficientMatches.size());
					avg_efficientMatchChunk += vec_efficientMatchChunk.back();

					if (max_efficientMatches < vec_efficientMatchChunk.back())
						max_efficientMatches = vec_efficientMatchChunk.back();
				}

				avg_orbkeypointsChunk /= no_chunks;
				avg_orbkeypointsFrame /= vec_orbkeypointsFrame.size();
				avg_efficientMatchChunk /= no_chunks;
				...
				ImGui::Text("current no. of Frames: %d", vec_orbkeypointsFrame.size());
				ImGui::Text("maximum no. of orbkeypoints: %d", max_orbFrame);
				ImGui::Text("average no. of orbkeypoints: %.2f", avg_orbkeypointsFrame);

				ImGui::NextColumn();
				ImGui::Text("current no. of Chunks: %d", vec_orbkeypointsChunk.size());
				ImGui::Text("maximum no. of orbkeypoints: %d", max_orbChunk);
				ImGui::Text("average no. of orbkeypoints: %.2f", avg_orbkeypointsChunk);

				ImGui::NextColumn();
				ImGui::Text("current no. of Chunks: %d", vec_efficientMatchChunk.size());
				ImGui::Text("maximum no. of efficient matches: %d", max_efficientMatches);
				ImGui::Text("average no. of efficient matches: %.2f", avg_efficientMatchChunk);
				ImGui::EndChild();
				ImGui::TreePop();
				ImGui::Separator();
			}
			...
			if (ImGui::TreeNode("Misc")) {
				ImGui::Text("mesh changed: %d", (bool)m.meshchanged);
				ImGui::TreePop();
				ImGui::Separator();
			}
		...
		if (ImGui::CollapsingHeader("PANDIA_INTEGRATION")) {
			...
			ImGui::Text("Buffer size of Integration: %d", pandia_integration::integrationBuffer.size());
			ImGui::Text("Buffer size of Reintegration: %d", pandia_integration::reintegrationBuffer.size());
			ImGui::Text("Buffer size of Deintegration: %d", pandia_integration::deintegrationBuffer.size());
			ImGui::Text("Buffer size of integratet frames: %d", pandia_integration::integratedframes.size());
		}


		if (ImGui::CollapsingHeader("TREADS")) {

			ImGui::Text("stopMeshing: %d", (bool)stopMeshing);
			ImGui::Text("current slam finished: %d", (bool)g_current_slam_finished);
			ImGui::Text("tracking lost: %d", (bool)g_trackingLost);
			ImGui::Text("recon-Thread finished: %d", (bool)g_reconThreadFinished);
			ImGui::Text("whole Mesh: %d", (bool)g_wholeMesh);
		}


		if (ImGui::CollapsingHeader("CAMERA")) {

			ImGui::Text("current Camera Typ:"); ImGui::SameLine();
			switch (g_camType) {
			...
			if (ImGui::TreeNode("current Camera Position")) {
			...
			if (ImGui::TreeNode("Intrinsic CUDA")) {
				ImGui::Text("fx: %.2f", g_intrinsic_cuda.fx_);
				ImGui::Text("fy: %.2f", g_intrinsic_cuda.fy_);
				ImGui::Text("w:  %.2f", g_intrinsic_cuda.cx_);
				ImGui::Text("h:  %.2f", g_intrinsic_cuda.cy_);
				ImGui::Text("camera width:  %d", g_intrinsic_cuda.width_);
				ImGui::Text("camera height: %d", g_intrinsic_cuda.height_);
				ImGui::TreePop();
				ImGui::Separator();
			}

			if (ImGui::TreeNode("Intrinsic Virtual")) {
				ImGui::Text("fx: %.2f", virtualIntrinsic.intrinsic_matrix_(0, 0));
				ImGui::Text("fy: %.2f", virtualIntrinsic.intrinsic_matrix_(1, 1));
				ImGui::Text("w:  %.2f", virtualIntrinsic.intrinsic_matrix_(0, 2));
				ImGui::Text("h:  %.2f", virtualIntrinsic.intrinsic_matrix_(1, 2));
				ImGui::Text("camera width:  %d", virtualIntrinsic.width_);
				ImGui::Text("camera height: %d", virtualIntrinsic.height_);
				ImGui::TreePop();
				ImGui::Separator();
			}
		...
		if (ImGui::CollapsingHeader("GUI")) {
			...
			ImGui::TextColored(col_orange, "Clock: %s", t.getClock());
